...
class AuthManager {
  ...
  async login(email: string, password: string): Promise<{ success: boolean; user?: User; message: string; session?: AuthSession }> {
    try {
      ...
      const expectedPassword = validPasswords[email] || 'Demo123!';
      ...
      dataStore.saveData({ ...data, users: updatedUsers });
  ...
  createDemoAccounts(): void {
    ...
    if (!adminExists || !parentExists) {
      dataStore.saveData({ ...data, users: newUsers });
    }
  }
}
