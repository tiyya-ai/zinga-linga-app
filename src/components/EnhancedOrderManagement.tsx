...
export const EnhancedOrderManagement: React.FC<EnhancedOrderManagementProps> = ({
  purchases,
  users,
  modules,
  searchTerm,
  ...
  const createManualOrder = () => {
    ...
    user.totalSpent += manualOrder.amount;
  ...
  const exportOrdersExcel = () => {
    // Create a more detailed export for Excel
    const excelData = purchases.map(p => {
      const user = users.find(u => u.id === p.userId);
      const purchasedModules = modules.filter(m => p.moduleIds?.includes(m.id));
      
      return {
        'Order ID': p.id.slice(-8).toUpperCase(),
        'Full Order ID': p.id,
        'Customer Name': user?.name || 'Unknown',
        'Customer Email': user?.email || 'No email',
        'Amount': p.amount.toFixed(2),
        'Status': p.status,
        'Payment Method': p.paymentMethod || 'card',
        'Order Type': p.isManual ? 'Manual' : 'Online',
        'Modules': purchasedModules.map(m => m.title).join('; '),
        'Module IDs': p.moduleIds?.join('; ') || '',
        'Order Date': new Date(p.createdAt).toLocaleDateString(),
        'Order Time': new Date(p.createdAt).toLocaleTimeString(),
        'Completed Date': p.completedAt ? new Date(p.completedAt).toLocaleDateString() : '',
        'Refund Date': p.refundedAt ? new Date(p.refundedAt).toLocaleDateString() : '',
        'Refund Reason': p.refundReason || '',
        'Notes': p.notes || '',
        'User Total Spent': user?.totalSpent?.toFixed(2) || '0.00',
        'User Join Date': user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : ''
      };
    });
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `zinga_linga_detailed_orders_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    window.URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
          <h3 className="text-3xl font-mali font-bold text-gray-800">Enhanced Order Management</h3>
          <p className="font-mali text-gray-600">Professional order tracking with manual order creation and advanced export</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-purple-600 text-white px-4 py-2 rounded-xl hover:bg-purple-700 transition-colors font-mali font-bold"
            >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              {selectedOrder.moduleIds && selectedOrder.moduleIds.length > 0 && (
                <div className="bg-blue-50 rounded-lg p-4">
                  <span className="font-mali text-gray-600">Purchased Modules:</span>
                  <div className="mt-2 space-y-2">
                    {selectedOrder.moduleIds.map(moduleId => {
                      const module = modules.find(m => m.id === moduleId);
                      return module ? (
                        <div key={moduleId} className="flex justify-between items-center">
                          <span className="font-mali font-bold">{module.title}</span>
                          <span className="font-mali text-green-600">${module.price}</span>
                        </div>
                      ) : null;
                    })}
                  </div>
