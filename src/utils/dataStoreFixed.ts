...
class DataStore {
  ...
  // Load data from localStorage or return default - PRESERVE USER CHANGES
  loadData() {
    try {
      ...
      if (stored) {
        ...
        // Return stored data as-is, only fill missing properties with defaults
        // DO NOT merge with defaultData to preserve user changes
        return {
          users: data.users || this.defaultData.users,
          modules: data.modules || this.defaultData.modules,
          purchases: data.purchases || this.defaultData.purchases,
          contentFiles: data.contentFiles || this.defaultData.contentFiles,
